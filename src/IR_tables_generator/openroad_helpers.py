...
class CircuitOps_File_DIR:
  def __init__(self, CircuitOps_dir, def_file, stage, seg, design, tech, out_dir):
    ...
    self.LIB_FILES = libDir.glob('*.lib')
    self.TECH_LEF_FILE = lefDir.glob("*tech*.lef")    
    self.SDC_FILE = self.DESIGN_DIR + "/"+self.DESIGN_NAME+".sdc"
    self.NETLIST_FILE = self.DESIGN_DIR +  "/"+self.DESIGN_NAME+".v"

    ### SET OUTPUT DIRECTORY ###
    if out_dir == "NaN":
        self.OUTPUT_DIR = self.CIRCUIT_OPS_DIR + "/designs/" + self.DESIGN_NAME + "/IRs/"
        self.create_path()
    else:
